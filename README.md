# ShiftMate_app
## サービス概要
本アプリケーションは、夜勤スタッフの仮眠休憩シフトを自動で振り分けできる管理ツールです。
スタッフや休憩室の条件を設定するだけで、最適なシフトを短時間で作成できます。
これにより、調整時間を大幅に削減し、管理業務の効率化を実現します。

## このサービスへの思い・作りたい理由
私は現在24時間稼働している職場で勤務をしており、夜勤中では複数のスタッフ（約8～10名）と複数の仮眠室に交代で仮眠休憩（2時間）をとっています。現在は管理者が手書きで仮眠休憩のシフト作成を行っており、複雑な場合であるとシフト作成にかなりの手間や時間を要していました。この作成作業をアプリケーション化することで作成時間や手間を短縮することができると考え、今回の発案に至りました。
